window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function useSidebarUpperHost(enabled) {
			...
			(0, react.useLayoutEffect)(() => {
				...
				while (foot !== null) {
					...
					if (candidate?.matches("[data-workspaces]") === true || candidate?.querySelector("[data-slot=\"sidebar.workspaces\"]") != null) break;
				...
				const keepImmediatelyAboveWorkspace = () => {
					if (nextHost.nextElementSibling !== workspaceRegion) sidebar.insertBefore(nextHost, workspaceRegion);
				};
		...
		function MissionControlNav({ wide, primaryPage, pageId, open, openState, placement = "inline" }) {
			const overlayOpen = (0, react.useSyncExternalStore)(openState?.subscribe ?? subscribeClosed, openState?.getSnapshot ?? getClosed, getClosed);
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				ref: marker,
				hidden: true
			}), host === null ? null : (0, react_dom.createPortal)(button, host)] });
		}
		//#endregion
...
`;
		...
		function MiniChatPane({ sessionId, session, directory, listCommands, openInMain }) {
			...
			const subscribeSession = (0, react.useMemo)(() => session === void 0 ? () => () => {} : (listener) => session.subscribe(listener), [session]);
			const snapshot = (0, react.useSyncExternalStore)(subscribeSession, () => session?.getSnapshot() ?? null, () => session?.getSnapshot() ?? null);
			(0, react.useEffect)(() => {
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							snapshot === null ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								...
								if (node.kind !== "user" && node.kind !== "steering" && node.kind !== "context") return null;
...
`;
		...
		function subscribeDocumentLanguage(listener) {
		...
		*/
		...
		const StableMiniChatPane = react.default.memo(MiniChatPane);
		...
		function PortalPane({ sessionId, title, cwd, row, defaultSize, rowHeight, session, directory, listCommands, openInMain }) {
			...
			const openSingle = (0, react.useCallback)(() => {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ResizablePane, {
				...
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(StableMiniChatPane, {
					sessionId,
					session,
					directory,
					listCommands,
		...
		function MissionControlPage({ useSessions, getSession, getModelDirectory, listCommands, openInMain }) {
			const uiLanguage = (0, react.useSyncExternalStore)(subscribeDocumentLanguage, getDocumentLanguage, () => "en");
			...
			const paneHostsRef = (0, react.useRef)(/* @__PURE__ */ new Map());
			const [viewport, setViewport] = (0, react.useState)(null);
			const availableIds = (0, react.useMemo)(() => sessions.ids.filter((id) => !panes.includes(id)), [sessions.ids, panes]);
			...
			const rowLayout = /* @__PURE__ */ new Map();
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						}), availableIds.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							style: {
								display: "flex",
								gap: 8,
								alignItems: "center"
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [panes.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", {
							...
							children: isChineseLanguage(uiLanguage) ? "将左侧边栏中的对话拖到这里，即可开始多对话窗口视图。" : "Drag a conversation here to start a multi-pane view."
						}) : rowNumbers.map((row, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(PaneRow, {
							row,
							ids: rowIdsByNumber[index] ?? [],
							hosts: paneHostsRef.current,
							height: rowLayout.get(row)?.height ?? FALLBACK_PANE_SIZE.height
						}, row)), panes.map((sessionId) => {
							...
							return (0, react_dom.createPortal)(/* @__PURE__ */ (0, react_jsx_runtime.jsx)(PortalPane, {
								sessionId,
		...
		//#endregion
		//#region src/client/MissionControlOverlay.tsx
		...
		function MissionControlOverlay({ close, openState, ...page }) {
			const open = (0, react.useSyncExternalStore)(openState.subscribe, openState.getSnapshot, openState.getSnapshot);
			const root = (0, react.useRef)(null);
			const [inset, setInset] = (0, react.useState)({
				left: 0,
				right: 0
			});
			(0, react.useLayoutEffect)(() => {
				if (!open) return;
				const frame = ((root.current?.parentElement)?.parentElement)?.parentElement;
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					...
					children: "×"
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(MissionControlPage, { ...page })]
			});
		}
		//#endregion
		//#region src/client/navigation.ts
		/** Create one plugin-lifecycle navigation controller. */
		function createMissionControlNavigation() {
			let open = false;
			const listeners = /* @__PURE__ */ new Set();
			const set = (next) => {
				if (open === next) return;
		...
		function dropSurface(target) {
			...
			const center = conversation.closest("[data-slot=\"conversation\"]")?.parentElement;
			return center instanceof HTMLElement && center.parentElement?.hasAttribute("data-details-collapsed") === true ? center : null;
		}
		...
		function apply(ctx) {
			...
			const onDragOver = (event) => {
				...
				if (!event.dataTransfer.types.includes("text/plain") && !event.dataTransfer.types.includes("application/x-mcp-pane")) return;
				event.preventDefault();
			...
			const onDrop = (event) => {
				...
				const dragged = paneDragged ? event.dataTransfer.getData(PANE_DRAG_MIME) : event.dataTransfer.getData("text/plain");
				if (dragged === "") return;
				event.preventDefault();
				clearDropPreview();
				...
				const before = rowElement === null ? void 0 : beforeIdForDrop(rowElement, event.clientX, dragged);
				if (paneDragged && rowElement !== null) {
					const draggedPane = document.querySelector(`[data-mcp-session="${CSS.escape(dragged)}"]`);
					if (!rowFitsAfterInsert(rowElement, dragged, draggedPane === null ? getPaneSize(dragged)?.width ?? 360 : draggedPane.getBoundingClientRect().width, before)) return;
				}
