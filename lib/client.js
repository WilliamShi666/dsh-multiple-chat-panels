...
`;
		...
		function MiniChatPane({ sessionId, session, directory, listCommands, openInMain }) {
			...
			const useSessionSnapshot = (0, react.useMemo)(() => session === void 0 ? null : (0, _deepseek_ai_dsh_client_web_react.bindSnapshotSelector)(session), [session]);
			const snapshot = useSessionSnapshot === null ? null : useSessionSnapshot((s) => s);
			(0, react.useEffect)(() => {
		...
		*/
		function desiredRowHeights(rowIdsByNumber, viewport) {
			...
			return rowIdsByNumber.map((ids) => {
				...
				for (const id of ids) {
					...
					if (persisted !== void 0 && persisted.height > tallest) tallest = persisted.height;
				}
				return tallest;
			});
		}
		...
		function ResizablePane({ sessionId, title, cwd, row, defaultSize, rowHeight, onClose, onOpenSingle, children }) {
			...
			const size = live !== null ? live : {
				...effectiveSize,
				height: Math.min(effectiveSize.height, rowHeight)
			};
			const nextSize = (clientX, clientY) => {
				const start = dragRef.current;
				if (start === null) return null;
				return {
					width: start.axis === "s" ? start.width : Math.max(360, start.width + clientX - start.x),
					height: start.axis === "e" ? start.height : Math.max(280, start.height + clientY - start.y)
				};
			};
			...
			const finishResize = (event, commit) => {
				...
				if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId);
				if (commit && next !== null && (next.width !== start.width || next.height !== start.height)) setPaneSize(sessionId, next);
				setLive(null);
			};
		...
		function apply(ctx) {
			...
			const onDragOver = (event) => {
				...
				if (target !== null) target.classList.add("mcp-drop-target");
				else grid.dataset.mcpNewRow = "1";
			};
			const onDrop = (event) => {
				if (!isCenterTarget(event.target)) return;
				if (event.dataTransfer === null) return;
