...
export function MiniChatPane({ sessionId, session, directory, listCommands, openInMain }) {
    ...
    const subscribeSession = useMemo(() => session === undefined
        ? () => () => { }
        : (listener) => session.subscribe(listener), [session]);
    const snapshot = useSyncExternalStore(subscribeSession, () => session?.getSnapshot() ?? null, () => session?.getSnapshot() ?? null);
    useEffect(() => {
