...
export function MissionControlPage({ useSessions, getSession, getModelDirectory, listCommands, openInMain, }) {
    ...
    const renderRow = (ids, row, rowHeight) => {
        ...
        return (_jsx("div", { "data-mcp-row": row, style: {
                display: 'flex',
                gap: PANE_GAP,
                alignItems: 'stretch',
                overflow: 'auto',
                ...
                return (_jsx(ResizablePane, { sessionId: sessionId, title: title, cwd: summary?.cwd, row: row, defaultSize: defaultSize, rowHeight: rowHeight, onClose: () => removePane(sessionId), onOpenSingle: () => openInMain(sessionId), children: _jsx(MiniChatPane, { sessionId: sessionId, session: getSession(sessionId), directory: getModelDirectory(sessionId), listCommands: listCommands, openInMain: () => openInMain(sessionId) }) }, sessionId));
            }) }));
    };
    return (_jsxs("div", { style: {
            ...
            overflow: 'hidden',
                    ...
                    flexShrink: 0,
                                    ...
                                    borderRadius: 6,
                                    fontWeight: 600,
                                }, children: "Add" })] }))] }), _jsx("div", { ref: gridRef, "data-mcp-grid": true, style: {
                    display: 'flex',
                    flexDirection: 'column',
                    gap: PANE_GAP,
                    flex: 1,
                    ...
                    overflowY: 'auto',
                    overflowX: 'hidden',
                }, children: panes.length === 0 ? (_jsx("p", { style: { color: 'var(--dsw-alias-label-primary-dimmed, #656d76)', margin: 0 }, children: "Drag a conversation here to start a multi-pane view." })) : rowNumbers.map((row, index) => renderRow(rowIdsByNumber[index] ?? [], row, rowHeights[index] ?? FALLBACK_PANE_SIZE.height)) })] }));
}
